...
public abstract class DefaultInteraction implements Interaction {
     ...
     */
    public DefaultInteraction(Elements elems) {
        if (elems != null) {
    ...
    protected Elements getFirst(Elements elems) {
        Elements first = elems.as(BasicWebElements.class).first();
        return first;
    }
    ...
    protected void wait(Elements webElements, Duration timeout, Duration interval, Predicate<? super Elements> predicate) {
        if (timeout == null) {
        ...
        Function<? super Elements, Boolean> function = Functions.forPredicate(predicate);
        wait.until(function);
    }
    ...
    protected void waitOrTimeout(Elements webElements, Duration timeout, Duration interval, Predicate<? super Elements> predicate) {
        if (timeout == null) {
            timeout = configure().defaultTimeout();
        }
        if (interval == null) {
