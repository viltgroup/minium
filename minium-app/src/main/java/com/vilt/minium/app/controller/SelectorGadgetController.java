...
public class SelectorGadgetController {
    ...
    public ResponseEntity<SelectorGadgetResult> getCssSelector(@PathVariable("var") String var) throws Exception {
        SelectorGadgetWebElements elems = getSelectorGadgetWebElements(var);
        SelectorGadgetResult result = new SelectorGadgetResult();
        result.setWebDriverVarName(var);
        String cssSelector = elems.getCssSelector();
        if (cssSelector != null) {
            result.setExpression(format("$(%s, \"%s\")", var, cssSelector));
        }
