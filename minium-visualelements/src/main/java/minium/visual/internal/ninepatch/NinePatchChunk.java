 ...
 *
 * This does not represent the bitmap, only the chunk info responsible for the padding and the stretching areas.
 *
 * Since android.graphics.drawable.NinePatchDrawable and android.graphics.NinePatch both deal with the nine patch chunk
...
public class NinePatchChunk implements Serializable {
     ...
     *
     * @param image
     *            the image containing both the content and the control outer line.
     ...
     *
     * This method is not thread-safe and returns an array owned by the {@link NinePatchChunk} class.
     *
    ...
    /**
     * Finds the 9-patch patches and padding from a {@link BufferedImage} image that contains both the image content and
     * the control outer lines.
     */
     ...
     *
     * The return value is a pair of list. The first list ({@link Pair#mFirst}) is the list of fixed area. The second
     * list ({@link Pair#mSecond}) is the list of stretchable areas.
     *
     * Each area is defined as a Pair of (start, end) coordinate in the given line.
     *
     * @param pixels
     *            the pixels of the control line. The line should have the same length as the content (i.e. it should be
     *            stripped of the first/last control pixel which are not used)
     * @param startWithPatch
     *            a boolean array of size 1 used to return the boolean value of whether a patch (stretchable area) is
     *            first or not.
     * @return
     */
